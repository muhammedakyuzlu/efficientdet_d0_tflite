#include "efficientdet_d0_tflite.h"


void EFFICIENTDET_D0::getLabelsName(std::string path, std::vector<std::string> &labelNames)
{
    // Open the File
...
void EFFICIENTDET_D0::loadModel(const  std::string path)
{
    ...
    _input_32 = _interpreter->typed_tensor<float>(_input);
...
void EFFICIENTDET_D0::preprocess(cv::Mat &image)
{
...
void EFFICIENTDET_D0::run(cv::Mat frame, Prediction &out_pred)
{
    cv::Size s = frame.size();

    preprocess(frame);

    memcpy(_input_32, frame.data, frame.total() * frame.elemSize());
    ...
    // bbox
    output_locations = _interpreter->tensor(_interpreter->outputs()[1]);
    auto out_location = output_locations->data.f;
    
    // label/class
    output_classes   = _interpreter->tensor(_interpreter->outputs()[3]);
    auto out_cls = output_classes->data.f;
    
    // scores
    output_scores   = _interpreter->tensor(_interpreter->outputs()[0]);
    auto out_score = output_scores->data.f;

    // number of detection
    num_detections   = _interpreter->tensor(_interpreter->outputs()[2]);
